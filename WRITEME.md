...
1. [bximage](https://bochs.sourceforge.io/doc/docbook/user/using-bximage.html)
...
2. [qemu](https://wiki.qemu.org/Main_Page)
...
3. [nasm assembler](https://www.nasm.us/)
...
4. [lba](https://en.wikipedia.org/wiki/Logical_block_addressing)
    
    Logical block addressing: 使用单一编号进行对不同磁盘分区进行索引，取代了传统的使用 cylinder-head-sector 方法。

5. [virtual-box](https://www.virtualbox.org/)
...
6. [intel-8086](https://en.wikipedia.org/wiki/Intel_8086#cite_note-3)
...
7. 先使用 bximage 制作了一块虚拟硬盘.img，然后使用 dd 命令将512字节的.bin 文件写入 .img , 为什么 bochs 就能够直接加载这个硬盘中的指令呢？同样的问题，为什么使用 qemu convert 转换为 vdi格式后，virtual_box也能直接打开呢？不需要一些对硬盘格式的说明吗？
...
8. [nasm 编译器语法](https://www.nasm.us/xdoc/2.16.02/html/nasmdoc0.html)
...
12. x86寄存器 [参考MCS-86 Family User’s Manual](https://edge.edx.org/c4x/BITSPilani/EEE231/asset/8086_family_Users_Manual_1_.pdf)
